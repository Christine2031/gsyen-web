...
// Models that respond with JSON instead of SSE stream.
const STRUCTURED_MODELS = new Set(['gemini', 'ethan', 'fast']);
...
/**
 * System-prompt suffix injected for structured-output models.
 * Tells the AI *when* to set shouldCreateEvent and *how* to fill the event.
 */
    ...
    try {
      ...
      if (model === 'gemini') {
        ...
        try {
          ...
          return res.json({ text: rawText, event: null });
        }
      }
      ...
      if (model === 'ethan' || model === 'fast') {
        ...
        try {
          ...
          return res.json({ text: rawContent, event: null });
        }
      }
