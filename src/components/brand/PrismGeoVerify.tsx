    ...
    <div className="bg-white border border-[#1A1A1A]/10 p-5 space-y-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {(zh ? PRESETS_ZH : PRESETS_EN).map(p => (
          <button key={p} onClick={() => run(p)}
            className="px-2 py-1 text-(--fs-xs) font-mono border border-[#1A1A1A]/12 text-[#1A1A1A]/60 hover:text-[#1A1A1A] hover:border-[#1A1A1A]/30 rounded-none transition-all">
            {p}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-2">
        <input type="text" value={query}
          ...
          className="flex-1 px-3 py-2 text-(--fs-md) font-mono border border-[#1A1A1A]/15 bg-[#F9F8F6] outline-none focus:border-[#1A1A1A] rounded-none transition" />
        ...
        <div className="border border-[#1A1A1A]/10 space-y-0 divide-y divide-[#1A1A1A]/8">
          ...
          <div className="p-4 flex items-center gap-6 bg-[#F9F8F6]">
            <div className="text-center">
              ...
              <p className="text-(--fs-2xs) font-mono uppercase tracking-widest text-[#1A1A1A]/40 mt-0.5">
            ...
            <div className="flex-1">
              {result.score >= 80
                ? <div className="flex items-center gap-1.5 text-emerald-700"><ShieldCheck className="w-4 h-4" /><span className="text-(--fs-xs) font-mono uppercase tracking-widest font-bold">{zh ? '核心事实锚定' : 'CORE FACT ANCHORED'}</span></div>
                : <div className="flex items-center gap-1.5 text-amber-600"><AlertTriangle className="w-4 h-4" /><span className="text-(--fs-xs) font-mono uppercase tracking-widest font-bold">{zh ? '语料补强建议' : 'CORPUS BOOST NEEDED'}</span></div>
              }
            </div>
          </div>
          ...
          <div className="p-4 space-y-1.5">
            <p className="text-(--fs-2xs) font-mono uppercase tracking-widest text-[#1A1A1A]/40">{zh ? 'AI 生成引用' : 'AI Citation'}</p>
            <p className="text-(--fs-md) text-[#1A1A1A]/80 font-mono leading-relaxed">{result.answer}</p>
          </div>

          {/* 注入语料块 */}
          {result.chunks.length > 0 && (
            <div className="p-4 space-y-2">
              <p className="text-(--fs-2xs) font-mono uppercase tracking-widest text-[#1A1A1A]/40 flex items-center gap-1.5">
              ...
              {result.chunks.map((c, i) => (
                <div key={i} className="bg-[#F4F2EE] px-3 py-2 border-l-2 border-[#1A1A1A]/20">
                  <p className="text-(--fs-xs) font-mono font-bold text-[#1A1A1A]">{c.title}</p>
                  <p className="text-(--fs-xs) font-mono text-[#1A1A1A]/60 line-clamp-2 mt-0.5">{c.content}</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="p-4 grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <p className="text-(--fs-2xs) font-mono uppercase tracking-widest text-emerald-700">{zh ? '可信事实' : 'Detected Facts'}</p>
              {result.facts.map((f, i) => <p key={i} className="text-(--fs-xs) font-mono text-[#1A1A1A]/70">✓ {f}</p>)}
            </div>
            {result.hype.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-(--fs-2xs) font-mono uppercase tracking-widest text-amber-600">{zh ? 'Slop 预警' : 'Slop Warnings'}</p>
                {result.hype.map((h, i) => <p key={i} className="text-(--fs-xs) font-mono text-[#1A1A1A]/70">⚠ {h}</p>)}
              </div>
            )}
          </div>
        </div>
