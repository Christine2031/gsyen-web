...
function assertWorkerInput(filePath, limits) {
  const stat = fs.statSync(filePath);
  if (!stat.isFile() || !Number.isSafeInteger(stat.size) ||
      stat.size > limits.maxInputBytes) {
...
async function parseOfficeFile(filePath, ext, limits) {
  assertWorkerInput(filePath, limits);
  if (ext === '.docx') {
...
if (!isMainThread) {
  parseOfficeFile(workerData.filePath, workerData.ext, workerData.limits)
